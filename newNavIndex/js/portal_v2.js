...
function flag2(flag) {
	...
	if (flag) {
		...
		$(function() {
			...
			localStorage.setItem("orgid_baobiao", $("#org").val());
...
function styleTable(table) {
	...
	if (table == "#zongjieTable") {
		$table.find('td').eq(0).css({
			maxWidth: "78px",
		});
		$table.find('td').eq(1).css({
			maxWidth: "145px",
		});
		$table.find('td').eq(2).css({
			maxWidth: "60px",
		});
		$table.find('td').eq(3).css({
			maxWidth: "80px",
		});

		$table.find('td:nth-child(4n+1) a').css({
			maxWidth: "78px",
		});
		$table.find('td:nth-child(4n+2) a').css({
			maxWidth: "145px",
		});
		$table.find('td:nth-child(4n+3) a').css({
			maxWidth: "60px",
		});
		$table.find('td:nth-child(4n) a').css({
			maxWidth: "80px",
		});
	}
	...
	if (table == "#jianduTable" || table == "#jianduzhixingTable") {
		...
		$table.find('td:nth-child(3n + 1)').css({
			textAlign: 'left'
		});
		$table.find('td:nth-child(3n + 2) ').css({
			textAlign: 'left'
		});
		$table.find('td:nth-child(3n+1) a').css({
			maxWidth: "44px"
		});
		$table.find('td:nth-child(3n+2) a').css({
			maxWidth: "221px"
		});
		$table.find('td:nth-child(3n+3) a').css({
			maxWidth: "97px"
		});
	...
	if (table == "#baojingTable") {
		$table.find('td:nth-child(4n+1)').css({
			maxWidth: "125px",
			textAlign: 'left'
		});
	}
	...
	if (table == '#JTbaojing') {
		$table.find('td:nth-child(3n + 3)').css({
			textAlign: 'center'
		})
	...
	if (table == "#shengchanbaobiao") {
		...
		$table.find('td:nth-child(4n + 2)').css({
			textAlign: 'left'
		});
	}
...
function baojing(unit_status) {
	...
	ajax(url, "baojingTable", ['T_P_NAME', 'PI_CODE', 'W_DATE', 'W_LEVEL']);
}
//集团定期实验
function jtdqsy() {
...
function prearData(data, columns, table, diff) {
	...
	if (table == "baojingTable") {
		min = data.length > 10 ? 10 : data.length;
	}
		...
		for (var j = 0; j < columns.length; j++) {
			...
			if (table == "baojingTable" || table == 'JTbaojing') {
				...
				if (d.T_P_NAME) {
					...
					htmlArray.push("<td class='ellipsis' style=\"" + color + "\"><a title=" + columnValue + " class='' href='" + ctx + "/main?xwl=23WPD5TO7GWR&showAddWindow=Y&ID=" + d.W_ID + "&tpName=" + tpName + "&tpCode=" + d.T_P_CODE + "'>" + columnValue + "</a></td>");
					//console.log(tpName)
				} else {
					continue;
				}
...
$(document).ready(
	function() {
 	...
 	function(){
		...
		//集团定期实验更多链接
		$("#wj_jt_dq").click(function() {
				$("#wj_jt_more").attr("href", ctx + "/newNavIndex/dingqishiyan/qchuizong.html")
			})
			//集团设备轮换更多链接
		$("#wj_jt_sb").click(function() {
				$("#wj_jt_more").attr("href", ctx + "/newNavIndex/shebeilunhuan/qchuizong.html")
			})
			//集团机组起停更多链接
		$("#wj_jt_qt").click(function() {
			$("#wj_jt_more").attr("href", ctx + "/jzqt/index.html")
		})
...
function shengchanbaobiao_show() {
	...
	$('.dianchang_view').find('a').css({
		backgrond: "#fff"
	});
	$('.dianchang_view').find("td").css({
		backgrond: '#fff'
	});
	shengchanbaobiao_jt()
}
...
function jigaihref() {
	if ($("#org").val() == 'c21834b4-1cb0-490f-a2a8-deeaf7f7e065') { //岱海发电
		$('#jigaiInf').attr({
			href: ctx + "/main?xwl=23Z3QFWUNBT8"
		})
	} else if ($("#org").val() == '472212af-1977-462b-a74a-a1f36ed6562d') { //宁东发电
		$('#jigaiInf').attr({
			href: ctx + "/main?xwl=23YYSVDX10YP"
		})
	}
}
